...
public class AviaSouls {
    ...
    public Ticket[] searchAndSortBy(String from, String to, Comparator<Ticket> comparator) {
        Ticket[] result = new Ticket[0]; // создаём массив для подходящих билетов

        for (Ticket ticket : tickets) {
            if (ticket.getFrom().equals(from)) { // проверка вылета
                if (ticket.getTo().equals(to)) { // проверка прилёта
                    // добавление билета в массив результата
        ...
        // сортируем по переданному компаратору
